...
class EnergyManagement(SingletonMixin, ConfigMixin, PredictionMixin, PydanticBaseModel):
    ...
    feedin_hours: Optional[NDArray[Shape["*"], float]] = Field(default=None, description="TBD")
    ...
    def set_parameters(
        ...
        self.feedin_hours = np.full(self.config.prediction.hours, 0.0)
    ...
    def set_akku_feedin_hours(self, ds: np.ndarray) -> None:
        self.feedin_hours = ds
    ...
    def simulate(self, start_hour: int) -> dict[str, Any]:
        ...
        feedin_hours = np.array(self.feedin_hours)
        ...
        for hour in range(start_hour, end_hour):
            ...
            if inverter:
                ...
                # Feed in
                if hour_feedin > 0.0:
                    energy_feedin_grid_static = inverter.max_power_wh * hour_feedin
